...
状态：草案（随 `2026-08-22-watch-face-v2-design.md` 评审）
...
flowchart TB
  ...
  subgraph Mac["Mac：TokenLink.app（单进程）"]
    subgraph App[TokenLinkApp]
      ...
      FH["FocusHandler<br/>聚焦 Codex 会话"]
    end
...
sequenceDiagram
  ...
  alt source == codex 且会话存在
    ...
    FH->>CD: 激活 Codex Desktop 并选中该 thread
    FH-->>AM: 记录完成事件
  else 非 Codex 来源 / 会话已不存在
    AM-->>AM: 记录事件，不向用户报错
  end
  Note over W,CD: 命令只带 slot 序号；会话与应用的对应关系只在 Mac 端
```
