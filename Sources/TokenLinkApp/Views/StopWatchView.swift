        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 22) {
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    Text("Bind one M5Stack device and sync the Codex primary quota window over the existing private GATT service.")
        ...
        HStack(alignment: .top, spacing: 13) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text("Firmware v1 compatibility mode")
                    .font(.headline)
                Text("The current face remains unchanged. TokenLink sends only Codex using `remaining_percent` and `reset_in_seconds`; Kimi, MiniMax, and GLM stay on the Mac until protocol v2.")
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
            }
        }
        ...
        VStack(alignment: .leading, spacing: 14) {
            HStack {
                ...
                Button {
                    ...
                    Label(model.isDiscovering ? "Scanning…" : "Scan", systemImage: "dot.radiowaves.left.and.right")
                }
