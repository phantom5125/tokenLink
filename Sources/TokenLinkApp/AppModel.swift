...
public final class AppModel {
    ...
    public static func live() -> AppModel {
        ...
        let codexURL = configuration.codexPath.map { URL(fileURLWithPath: $0) }
    ...
    public var highlight: Highlight? {
        rows.compactMap { row -> Highlight? in
            guard row.phase == .healthy || row.phase == .stale,
                  ...
                  let window = snapshot.mostConstrainedWindow else { return nil }
    ...
    public func refreshManually() async {
        ...
        if let last = lastManualRefresh,
           now.timeIntervalSince(last) < Self.manualThrottleSeconds { return }
    ...
    private func reloadFromStore() async {
        ...
        rows = ProviderID.allCases.map { id in
            ...
            return ProviderRow(provider: id, phase: state.phase, snapshot: state.snapshot, error: state.error)
        }
    }
    ...
    public func syncCodexNow() async {
        ...
        guard let snapshot = state.snapshot,
              state.phase == .healthy || state.phase == .stale else {
