...
public final class AppModel {
    ...
    public static func preview(snapshots: [QuotaSnapshot]) -> AppModel {
        ...
        model.states = Dictionary(uniqueKeysWithValues: snapshots.map {
    ...
    public static func live() -> AppModel {
        ...
        if configuration.enabledProviders.contains(.codex) {
            providers.append(CodexProvider(executable: resolveCodexExecutable(
    ...
    public var menuBarLabel: String {
        ...
        return "\(Self.displayName(for: highlight.provider)) \(Int(highlight.window.remainingPercent.rounded()))%"
    }
    ...
    public func refreshManually() async {
        ...
        if let lastManualRefresh,
           date.timeIntervalSince(lastManualRefresh) < 10 { return }
    ...
    public func setProvider(_ provider: ProviderID, enabled: Bool) throws {
        if enabled { configuration.enabledProviders.insert(provider) }
        else { configuration.enabledProviders.remove(provider) }
    ...
    public func diagnosticObject() -> [String: Any] {
        [
            ...
            "providers": orderedProviderRows.map { row in
                let remaining: Any = if let value = row.state.snapshot?
                    .mostConstrainedWindow?.remainingPercent { value } else { NSNull() }
                let fetchedAt: Any = if let snapshot = row.state.snapshot {
                    ISO8601DateFormatter().string(from: snapshot.fetchedAt)
                } else { NSNull() }
                let errorKind: Any = if let value = row.state.error?.kind.rawValue {
                    value
                } else { NSNull() }
                return [
    ...
    private static func resolveCodexExecutable(configuredPath: String?) -> URL {
        ...
        let pathEntries = ProcessInfo.processInfo.environment["PATH"]?
            .split(separator: ":")
            .map(String.init) ?? []
        let candidates = pathEntries.map { "\($0)/codex" } + [
