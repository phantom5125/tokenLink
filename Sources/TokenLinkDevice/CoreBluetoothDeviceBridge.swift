...
public final class CoreBluetoothTransport: NSObject, BLETransport, @unchecked Sendable {
  ...
  public func discoveredIdentifiers() async throws -> [UUID] {
    guard central.state == .poweredOn else {
      throw CoreBluetoothTransportError.bluetoothUnavailable
    }
  ...
  private func ensureWriteCharacteristic(on peripheral: CBPeripheral) async throws
    -> CBCharacteristic
  {
    if let cached = lock.withLock({ writeCharacteristic }) { return cached }
    guard let service = peripheral.services?.first(where: { $0.uuid == Self.quotaServiceUUID })
    else {
  ...
  private func discoverWriteCharacteristic(
    ...
    try await Task.sleep(for: .milliseconds(500))
    guard
      let characteristic = service.characteristics?.first(where: { $0.uuid == Self.quotaWriteUUID })
    else {
