...
@Test func kimiCLIReaderUsesOnlyDocumentedCredentialFile() async throws {
    ...
    let credentialDirectory = root.appending(path: ".kimi-code/credentials", directoryHint: .isDirectory)
    ...
    try Data(#"{"access_token":"safe-token","expires_at":4102444800,"refresh_token":"never-return-this"}"#.utf8)
        .write(to: credential)
...
@Test func kimiProviderPrefersExplicitAPIKey() async throws {
    let http = KimiHTTPClient(response: HTTPResponse(
