...
class CodexMicroBle {
 public:
  ...
  static constexpr char kFirmwareVersion[] = "0.2.2-tokenlink";
  ...
  // Watch -> Mac command channel (C04). The watch is the GATT server, so the
  // BLE-idiomatic direction for server-to-client data is NOTIFY on a readable
  // characteristic; TokenLink subscribes on the Mac side.
  void sendWatchCommand(const char* action, int8_t slot = -1);
  bool connected();
