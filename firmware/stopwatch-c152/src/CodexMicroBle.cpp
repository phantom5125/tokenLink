...
constexpr char kCapabilitiesUuid[] = "7f0d4e66-2ac2-4a71-bfbe-4ef61a0e5c03";
// C04 is the watch -> Mac command channel. The watch is the GATT server, so
// per BLE convention server-to-client data uses NOTIFY on a readable
// characteristic: TokenLink reads or subscribes, the firmware updates the
// value and notifies. The Mac never writes this characteristic.
constexpr char kCommandUuid[] = "7f0d4e66-2ac2-4a71-bfbe-4ef61a0e5c04";
...
bool markGattMigrationPending() {
  ...
  const bool marked =
      preferences.putUChar("gatt-rev", kGattRevision) == sizeof(uint8_t);
  preferences.end();
  return marked;
}

void clearIncompatibleBonds() {
...
void CodexMicroBle::begin() {
...
#if defined(CONFIG_BLUEDROID_ENABLED)
  // GATT v2 changes the encrypted attribute table. Clear the old bond exactly
  // once so macOS pairs against C02/C03/C04 instead of retrying stale keys.
  if (migrateGattBond) clearIncompatibleBonds();
#endif
  ...
  BLEService* quotaService =
      server->createService(BLEUUID(String(kQuotaServiceUuid)), 10);
#else
  ...
  // C03 capabilities: TokenLink reads this after connecting to decide between
  // the v2 payload schema and the v1 fallback. Same encrypted link as C02.
  ...
  // command, NOTIFY so subscribed Macs receive focus/refresh commands
  // immediately. The firmware is the only writer of this value.
  command_ = quotaService->createCharacteristic(
      kCommandUuid,
      BLECharacteristic::PROPERTY_READ | BLECharacteristic::PROPERTY_NOTIFY);
  command_->setAccessPermissions(ESP_GATT_PERM_READ_ENCRYPTED);
  auto* commandCccd = new BLE2902();
  commandCccd->setAccessPermissions(ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE);
  command_->addDescriptor(commandCccd);
  command_->setValue("{}");
#if defined(CODEX_STOPWATCH_USB_MIC) && defined(CONFIG_BLUEDROID_ENABLED)
  ...
  Serial.printf("Quota GATT ready service=%s write=%s capabilities=%s "
                "command=%s\n",
                kQuotaServiceUuid, kQuotaWriteUuid, kCapabilitiesUuid,
                kCommandUuid);
}
...
void CodexMicroBle::processQuotaWrite(const uint8_t* data, size_t length,
  ...
  const JsonObjectConst updateObject = update.as<JsonObjectConst>();

  // Protocol v2 payload: richer watch-face data. Falls through to the v1
  // snapshot parser when the "v" field is absent, keeping the legacy Mac
  // companion fully compatible.
  if (watch_v2::isV2Payload(updateObject)) {
    watch_v2::Payload payload;
    if (!watch_v2::parse(updateObject, payload)) {
      Serial.println("Quota v2 update rejected: invalid fields");
      return;
    }
    xSemaphoreTake(stateMutex_, portMAX_DELAY);
    ...
    xSemaphoreGive(stateMutex_);
    Serial.printf(
        "Quota v2 update provider=%s windows=%u work_items=%u\n",
        payload.providerId, payload.windowCount, payload.workItemCount);
    return;
  }

  quota_payload::Snapshot snapshot;
  if (!quota_payload::parse(updateObject, snapshot)) {
